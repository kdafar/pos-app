...
export function PromoDialog() {
  ...
  const { promos, actions } = useOrderContext();
    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className={`${bg} border ${border} rounded-xl w-full max-w-md p-4`}>
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={actions.closePromoDialog} className={theme === 'dark' ? 'text-slate-400 hover:text-white' : 'text-gray-400 hover:text-gray-900'}>
            <X size={22} />
          </button>
        </div>
            ...
            <div>
              ...
              <div className="space-y-2 max-h-48 overflow-y-auto">
                {promos.filter((p: Promo) => p.active).map((promo: Promo) => (
                  <button
                    key={promo.id}
                    onClick={() => actions.applyPromoCode(promo.code)}

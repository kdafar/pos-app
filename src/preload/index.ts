...
const bridge = {
  ...
  sync: {
    configure: (baseUrl: string) =>
      ipcRenderer.invoke('sync:configure', baseUrl),
    bootstrap: (baseUrl?: string | null) =>
      ipcRenderer.invoke('sync:bootstrap', baseUrl ?? null), // ← one arg only
    pull: () => ipcRenderer.invoke('sync:pull'),
    push: (envelope: any, batch: any) =>
      ipcRenderer.invoke('sync:push', envelope, batch),
  ...
  auth: {
    ...
    loginWithPassword: (login: string, password: string) =>
      ipcRenderer.invoke('auth:loginWithPassword', login, password),
